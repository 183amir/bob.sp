 ...
 *
 * @brief Python bindings to DCT1D class methods
 *
...
static auto s_dct1d = bob::extension::ClassDoc(
  BOB_EXT_MODULE_PREFIX ".DCT1D",
  "Calculates the direct DCT of a 1D array/signal",
  "Input and output arrays are 1D NumPy arrays of type ``float64``."
)
.add_constructor(bob::extension::FunctionDoc(
  "DCT1D",
  "Creates a new DCT1D extractor in the given dimensions"
)
.add_prototype("length", "")
.add_prototype("other", "")
.add_parameter("length", "int", "The length of the input signal this class will be able to handle")
.add_parameter("other", ":py:class:`DCT1D`", "The other DCT1D class to copy-construct")
);
...
static void PyBobSpDCT1D_Delete (PyBobSpDCT1DObject* self) {
  self->cxx.reset();
  Py_TYPE(self)->tp_free((PyObject*)self);
}

static int PyBobSpDCT1D_InitCopy(PyBobSpDCT1DObject* self, PyObject* args, PyObject* kwds) {
  char** kwlist = s_dct1d.kwlist(1);

  PyBobSpDCT1DObject* other;
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O!", kwlist, &PyBobSpDCT1D_Type, &other)) return -1;

  self->cxx.reset(new bob::sp::DCT1D(*(other->cxx)));
  return 0;
}

static int PyBobSpDCT1D_InitShape(PyBobSpDCT1DObject* self, PyObject *args, PyObject* kwds) {
  char** kwlist = s_dct1d.kwlist(0);

  Py_ssize_t length = 0;
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "n", kwlist, &length)) return -1;

  self->cxx.reset(new bob::sp::DCT1D(length));
  return 0; ///< SUCCESS
}

static int PyBobSpDCT1D_Init(PyBobSpDCT1DObject* self, PyObject* args, PyObject* kwds) {
BOB_TRY
...
static PyObject* PyBobSpDCT1D_RichCompare (PyBobSpDCT1DObject* self, PyObject* other, int op) {
BOB_TRY
...
static PyObject* PyBobSpDCT1D_GetLength(PyBobSpDCT1DObject* self, void* /*closure*/) {
BOB_TRY
  return Py_BuildValue("n", self->cxx->getLength());
BOB_CATCH_MEMBER("length", 0)
}

static int PyBobSpDCT1D_SetLength(PyBobSpDCT1DObject* self, PyObject* o, void* /*closure*/) {
BOB_TRY
...
static PyObject* PyBobSpDCT1D_GetShape(PyBobSpDCT1DObject* self, void* /*closure*/) {
BOB_TRY
  return Py_BuildValue("(n)", self->cxx->getLength());
BOB_CATCH_MEMBER("shape", 0)
}

static int PyBobSpDCT1D_SetShape(PyBobSpDCT1DObject* self, PyObject* o, void* /*closure*/) {
BOB_TRY
...
;
static PyObject* PyBobSpDCT1D_transform(PyBobSpDCT1DObject* self, PyObject* args, PyObject* kwds) {
BOB_TRY
...
static PyMethodDef PyBobSpDCT1D_methods[] = {
    {
      s_transform.name(),
      (PyCFunction)PyBobSpDCT1D_transform,
...
bool init_BobSpDCT1D(PyObject* module){
  // class definition
  PyBobSpDCT1D_Type.tp_name = s_dct1d.name();
  PyBobSpDCT1D_Type.tp_basicsize = sizeof(PyBobSpDCT1DObject);
  PyBobSpDCT1D_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE;
  PyBobSpDCT1D_Type.tp_doc = s_dct1d.doc();

  PyBobSpDCT1D_Type.tp_new = PyType_GenericNew;
  PyBobSpDCT1D_Type.tp_dealloc = reinterpret_cast<destructor>(PyBobSpDCT1D_Delete);
  PyBobSpDCT1D_Type.tp_init = reinterpret_cast<initproc>(PyBobSpDCT1D_Init);
  PyBobSpDCT1D_Type.tp_richcompare = reinterpret_cast<richcmpfunc>(PyBobSpDCT1D_RichCompare);
  PyBobSpDCT1D_Type.tp_methods = PyBobSpDCT1D_methods;
  PyBobSpDCT1D_Type.tp_getset = PyBobSpDCT1D_getseters;
  PyBobSpDCT1D_Type.tp_call = reinterpret_cast<ternaryfunc>(PyBobSpDCT1D_transform);
  PyBobSpDCT1D_Type.tp_repr = reinterpret_cast<reprfunc>(PyBobSpDCT1D_Repr);

  // check that everything is fine
  if (PyType_Ready(&PyBobSpDCT1D_Type) < 0) return false;
  return PyModule_AddObject(module, "DCT1D", Py_BuildValue("O", &PyBobSpDCT1D_Type)) >= 0;
}
